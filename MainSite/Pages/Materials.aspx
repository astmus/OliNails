            ...
            <table>
                <tr>
                    <td style="vertical-align: top">
                        <asp:GridView ID="materialTable" OnRowCommand="materialTable_RowCommand" OnSelectedIndexChanged="table_SelectedIndexChanged" OnRowEditing="materialTable_RowEditing" OnRowUpdating="materialTable_RowUpdating" OnRowDataBound="table_RowDataBound" runat="server" AutoGenerateColumns="False" DataKeyNames="id" DataSourceID="materialDataSource" SelectedRowStyle-CssClass="selectedrow" CssClass="mydatagrid" PagerStyle-CssClass="pager" HeaderStyle-CssClass="header" RowStyle-CssClass="rows">
                            <Columns>
                                <asp:BoundField DataField="name" ReadOnly="true" ItemStyle-HorizontalAlign="Left" HeaderText="имя" SortExpression="name">
                                    <ItemStyle HorizontalAlign="Left"></ItemStyle>
                                </asp:BoundField>
                                <asp:BoundField DataField="startTime" ReadOnly="true" DataFormatString="{0:dd.MM.yyyy}" HeaderText="дата" SortExpression="startTime" />
                                <asp:BoundField DataField="price" HeaderText="цена" SortExpression="price" />
                                <asp:BoundField DataField="amount" HeaderText="ед." SortExpression="amount" />
                                <asp:TemplateField HeaderText="услуги" SortExpression="procedures">                                    
                                    <ItemTemplate>
                                        <asp:Label ID="Label1" runat="server" Text='<%# Bind("procedures") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:CommandField ShowSelectButton="true" SelectText="_" HeaderStyle-CssClass="hidden" FooterStyle-CssClass="hidden" ItemStyle-CssClass="hidden">
                        ...
                        <asp:SqlDataSource ID="materialDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:dbConnectionSctring %>" SelectCommand="SELECT * FROM [MaterialsWithRelatedProcedures] WHERE expiredTime IS NULL order by startTime desc" />
                    </td>
                    <td style="vertical-align: top">
